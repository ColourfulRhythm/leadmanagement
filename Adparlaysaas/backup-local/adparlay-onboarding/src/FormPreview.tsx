...
const FormPreview: React.FC = () => {
    ...
    const fetchForm = async () => {
      if (formId) {
        try {
          ...
          console.log('🔍 FormPreview: Using Supabase FormsService directly');
          ...
          // Go directly to Supabase, bypass any other API systems
          const foundForm = await FormsService.getForm(formId);
          console.log('🔍 FormPreview: Supabase form fetch result:', foundForm);
          
          if (foundForm) {
            setForm(foundForm);
          } else {
            console.log('❌ FormPreview: Form not found in Supabase');
            setForm(null);
          }
        } catch (error) {
          console.error('❌ FormPreview: Failed to fetch form from Supabase:', error);
          setForm(null);
        }
      }
      setLoading(false);
  ...
  if (!form.blocks || !form.questions) return (
    <div className="min-h-screen flex items-center justify-center text-xl">
      <div className="text-center">
        <h1 className="text-2xl font-bold text-red-600 mb-4">Invalid Form Data</h1>
        <p className="text-gray-600">The form data is corrupted or incomplete.</p>
      </div>
    </div>
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await FormsService.saveFormResponse({ 
        form_id: formId, 
